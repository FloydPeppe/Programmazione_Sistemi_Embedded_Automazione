...
traj_meas= scatter(NaN,NaN,'.');
traj_goal= scatter(NaN,NaN,'.');

axis equal
axis([-2.5*1.8 2.5*1.8 -1.8 1.8])
...
%}    
    ...
    try
        % Converting values
        input_L_real=  str2double(str{1});
        input_R_real=  str2double(str{2});
        input_L_goal=  str2double(str{3});
        input_R_goal=  str2double(str{4});
        L_real=        str2double(str{5});
        R_real=        str2double(str{6});
        L_goal=        str2double(str{7});
        R_goal=        str2double(str{8});
        xa_real=       str2double(str{9});
        ya_real=       str2double(str{10});
        theta_real=    str2double(str{11});
        xa_goal=       str2double(str{12});
        ya_goal=       str2double(str{13});
        theta_goal=    str2double(str{14});
        if OBSTACLE_AVOIDANCE
    ...
    fprintf("%.1f\t",input_L_real);
    fprintf("%.1f\t",input_R_real);
    fprintf("%.1f\t",L_real);
    fprintf("%.1f\t",R_real);
    fprintf("%.1f\t",L_goal);
    fprintf("%.1f\t",R_goal);
    ...
    xa_real_hist= [xa_real_hist xa_real];
    ya_real_hist= [ya_real_hist ya_real];
    xa_goal_hist= [xa_goal_hist xa_goal];
    ya_goal_hist= [ya_goal_hist ya_goal];
    
    % Draw RealTime trajectory lines
    set(traj_meas,'XData',xa_real_hist,'YData',ya_real_hist)
    set(traj_goal,'XData',xa_goal_hist,'YData',ya_goal_hist)
...
if IDENTIFICATION_ONLY
    ...
    % Call Reference Generator Script to update scripts
    ReferenceGenerator;
    return
end
